...
type Config struct {
	// SourcePath of the file to query
	SourcePath string
	// Query to apply
	Query string
...
// Service loop of the JSONQuery task.
type Service struct {
	Config
	log       zerolog.Logger
...
func (s *Service) Run(ctx context.Context) error {
	...
	// Parse query
	q, err := jq.Parse(s.Config.Query)
	if err != nil {
		log.Error().Err(err).Str("query", s.Config.Query).Msg("Failed to parse query")
		return maskAny(err)
	}

	// Open source file
	content, err := ioutil.ReadFile(s.Config.SourcePath)
	if err != nil {
	...
	// Publish result
	dataURI := dataurl.New(value, "text/plain")
