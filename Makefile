...
docker-patch-config:
	...
	@sed -e 's!image: .*!image: '"$(shell docker inspect --format="{{index .RepoDigests 0}}" $(SERVICESIMG))"'!' $(PATCHESDIR)/annotatedvalue_registry_stub_image_patch.yaml > $(OPERATOROVERLAYDIR)/annotatedvalue_registry_stub_image_patch.yaml
